...
exports.getWeeklyStats = onCall({ region: "asia-northeast2" }, async (request) => {
    try {
        const statsRef = getRtdb().ref('stats/dailyCounts');
        const dates = [];
        for (let i = 6; i >= 0; i--) {
            const d = new Date();
...
exports.searchHistory = onCall({ region: "asia-northeast2" }, async (request) => {
    ...
    try {
        const snapshot = await getHirusupaDb().collection("history").orderBy("sentAt", "desc").limit(5000).get();
...
exports.generateRadioNamesFromGemini = onCall({ region: "asia-northeast2" }, async (request) => {
    ...
    const mansukeUserRef = getUsersDb().collection('users').doc(uid);
    ...
    try {
        ...
        await getUsersDb().runTransaction(async (transaction) => {
        ...
        if (receiptAmount > 0) {
            try {
                await getUsersDb().runTransaction(async (transaction) => {
                    const rSnap = await transaction.get(receiptRef);
                    if (rSnap.exists && rSnap.data().used && !rSnap.data().refunded) {
                        ...
                        transaction.update(mansukeUserRef, {
                            balance: admin.firestore.FieldValue.increment(receiptAmount),
    ...
    try {
        ...
        const hirusupaUserRef = getHirusupaDb().collection('users').doc(uid);
        await hirusupaUserRef.set({
            geminiUsage: { [yearMonth]: admin.firestore.FieldValue.increment(1) },
...
exports.logSubmission = onCall({ region: "asia-northeast2" }, async (request) => {
    ...
    try {
        // 1. Firestore履歴保存
        await getHirusupaDb().collection("history").add({
            uid,
            artistName,
            songName,
            radioName,
        ...
        const totalRef = getRtdb().ref('stats/totalCount');
        const todayRef = getRtdb().ref(`stats/dailyCounts/${today}`);
...
exports.saveRadioName = onCall({ region: "asia-northeast2" }, async (request) => {
    ...
    try {
        await getHirusupaDb().collection("radio_names").add({
            uid,
            name,
