...
const archiveGame = async (t, roomRef, roomData, players, endStatus, winner = null) => {
    ...
    const historyRef = getDb().collection('artifacts').doc('mansuke-jinro').collection('public').doc('data').collection('match_history').doc(roomData.matchId);

    // チャット履歴の取得 (全体チャットとチームチャット)
    // サーバーサイドで取得してアーカイブデータに含める
    // ※トランザクション内でのコレクション取得制限と「Read after Write」回避のため正規のgetを使用
...
const applyPhaseChange = async (t, roomRef, room, players, options = {}) => {
    let next = "", logs = [], updates = { ...options }, batchOps = [], effects = [];

    // カウントダウン終了 → 役職確認
    if (room.phase === 'countdown') {
        const roleSettings = room.roleSettings || {};
        const roleRequests = room.roleRequests || {};
        const assignments = [];
        ...
        // 【厳格な安全装置】役職の数とプレイヤーの数が一致しない場合はゲームを強制終了する
        if (remainingRoles.length !== players.length) {
            logs.push({
                text: `【致命的なエラー】参加人数（${players.length}人）と役職の総数（${remainingRoles.length}）が一致しませんでした。ゲームの公平性を保つため、強制終了します。`,
                phase: "System",
                day: 1
            });
            updates.status = 'aborted'; // ステータスを中断に変更
            next = null;
        } else {
            // 1. 役職指定リクエストの処理
            const requestsByRole = {};
            Object.entries(roleRequests).forEach(([uid, reqData]) => {
                const role = typeof reqData === 'object' ? reqData.roleId : reqData;
                const receiptId = typeof reqData === 'object' ? reqData.receiptId : null;
                if (!requestsByRole[role]) requestsByRole[role] = [];
                requestsByRole[role].push({ uid, receiptId });
            });

            // 役職指定の結果を保持するマップ
            const roleRequestResults = {};

            // 役職ごとに抽選
            for (const role of Object.keys(roleSettings)) {
                const requesters = requestsByRole[role] || [];
                const availableSlots = remainingRoles.filter(r => r === role).length;

                // 抽選（シャッフルして前から取る）
                const shuffledRequesters = requesters.sort(() => Math.random() - 0.5);
                const winnersCount = Math.min(shuffledRequesters.length, availableSlots);
                const winners = shuffledRequesters.slice(0, winnersCount);

                for (const req of winners) {
                    const uid = req.uid;
                    const receiptId = req.receiptId;
                    const player = unassignedPlayers.find(p => p.id === uid);
                    if (player) {
                        assignments.push({ id: uid, role: role, name: player.name, charged: true });
                        // unassignedPlayersから削除
                        const idx = unassignedPlayers.findIndex(p => p.id === uid);
                        unassignedPlayers.splice(idx, 1);
                        // remainingRolesから1つ削除
                        const rIdx = remainingRoles.findIndex(r => r === role);
                        remainingRoles.splice(rIdx, 1);
                        ...
                        if (receiptId) {
                            ...
                            // 下位互換性 (古いクライアントからのリクエスト)
                            effects.push({
                                type: 'charge',
                                uid: uid,
                                amount: -10,
                                message: `[WEREWOLF] 役職指定（試合ID: ${room.matchId}）`,
                                idempotencyKey: `role_request_${room.matchId}_${uid}`
                            });
                        }
                        roleRequestResults[uid] = `【役職指定】10円を支払い、ご希望の役職「${ROLE_NAMES[role]}」に決定しました。`;
                    }
                }

                // 外れた人への通知
                const losers = shuffledRequesters.slice(winnersCount);
                losers.forEach(req => {
                    if (req.receiptId) {
                        effects.push({
                            type: 'refundReceipt',
                        ...
                        roleRequestResults[req.uid] = `【役職指定】ご希望の役職「${ROLE_NAMES[role]}」は定員を超過した為、抽選により外れました。料金は全額返金されました。`;
                    } else {
                        roleRequestResults[req.uid] = `【役職指定】ご希望の役職「${ROLE_NAMES[role]}」は定員を超過した為、抽選により外れました。料金は請求されません。`;
                    }
                });
            }

            // 2. 残りのプレイヤーにランダムで割り当て
            const shuffledRemainingRoles = remainingRoles.sort(() => Math.random() - 0.5);
            unassignedPlayers.forEach((player, i) => {
                const role = shuffledRemainingRoles[i];
                assignments.push({ id: player.id, role: role, name: player.name, charged: false });
            });

            // 3. 役職情報の保存（元の startGameHandler のロジックを移植）
            assignments.forEach(p => {
                ...
                const roleDataUpdate = {
                    role: p.role,
                    teammates: mates,
                    originalRole: p.role,
                    elderShield: p.role === 'elder'
                };

                // 役職指定の結果があれば追加
                if (roleRequestResults[p.id]) {
                    roleDataUpdate.roleRequestResult = roleRequestResults[p.id];
                }

                batchOps.push({
                    ref: secretRef,
                    data: roleDataUpdate,
                    merge: true // ★修正: 新規作成時は必ずmerge: trueにする必要があります
                });

                // メモリ上のプレイヤー情報も更新（applyPhaseChange内で後続の処理がある場合のため）
                const playerInList = players.find(pl => pl.id === p.id);
                if (playerInList) playerInList.role = p.role;
            });
    ...
    // 更新実行
    if (next !== "" || updates.status === 'finished' || updates.status === 'aborted') {
