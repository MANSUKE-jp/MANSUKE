...
import { X, Loader2, Edit3, Ban, CheckCircle2, Clock, Zap, Gift, User } from 'lucide-react';
...
const CardDetailModal = ({ card, onClose, onUpdated }) => {
    ...
    useEffect(() => {
        const fetchDetail = async () => {
            try {
                ...
                const result = await fn({ cardId: card.id });
    ...
    const handleUpdateBalance = async () => {
        ...
        try {
            ...
            await fn({ cardId: card.id, amount: amt });
            onUpdated?.();
            const refreshFn = callFunction('staffGetCardDetail');
            const result = await refreshFn({ cardId: card.id });
            setDetail(result.data);
    ...
    const st = statusMap[detail?.status || card.status] || statusMap.inactive;
    const events = [];
    const cd = detail || card;
    ...
    if (cd.disabledAt) events.push({ label: 'コード無効化', date: cd.disabledAt, dot: 'red', icon: Ban });
    if (cd.reactivatedAt) events.push({ label: 'コード有効化', date: cd.reactivatedAt, dot: 'green', icon: CheckCircle2 });
                ...
                {loading ? (
                    ...
                    <>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 24 }}>
                            <div className="modal-title" style={{ marginBottom: 0, flex: 1 }}>カード詳細</div>
                        ...
                        {/* Signatures */}
                        {(cd.customerSignature || cd.employeeSignature) && (
                            <div className="section-card" style={{ marginBottom: 24 }}>
                                <div className="section-header"><Edit3 size={16} /> 購入時署名</div>
                                <div className="section-body" style={{ display: 'flex', gap: 16 }}>
                                    {cd.customerSignature && (
                                        <div style={{ flex: 1 }}>
                                            <div style={{ fontSize: 'var(--xs)', color: 'var(--text-3)', marginBottom: 8 }}>お客様署名</div>
                                            <div style={{ background: '#fff', border: '1px solid var(--border)', borderRadius: 8, padding: 8, textAlign: 'center' }}>
                                                <img src={cd.customerSignature} alt="お客様署名" style={{ maxWidth: '100%', maxHeight: 100, objectFit: 'contain' }} />
                                            </div>
                                        </div>
                                    )}
                                    {cd.employeeSignature && (
                                        <div style={{ flex: 1 }}>
                                            <div style={{ fontSize: 'var(--xs)', color: 'var(--text-3)', marginBottom: 8 }}>担当者署名</div>
                                            <div style={{ background: '#fff', border: '1px solid var(--border)', borderRadius: 8, padding: 8, textAlign: 'center' }}>
                                                <img src={cd.employeeSignature} alt="担当者署名" style={{ maxWidth: '100%', maxHeight: 100, objectFit: 'contain' }} />
                                            </div>
                                        </div>
                                    )}
                                </div>
                        ...
                        {/* Toggle status for inactive/active/disabled */}
