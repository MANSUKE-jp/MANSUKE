...
export const GameScreen = ({ user, mansukeUser, room, roomCode, players, myPlayer, setView }) => {
    ...
    const advanceRetryCount = useRef(0); // 連続エラー時のリトライ制御
    ...
    useEffect(() => {
        ...
        // チャット監視
        const unsubChat = onSnapshot(query(collection(db, 'artifacts', 'mansuke-jinro', 'public', 'data', 'rooms', roomCode, 'chat'), orderBy('createdAt', 'asc')), (snap) => {
            setMessages(snap.docs.map(d => d.data()));
        }, () => {
            // silent
        });

        // 霊界チャット監視は別のuseEffectに移動 (生存者はアクセス権がないため)

        let ur = () => { };
        let ures = () => { };
        let roleResultShown = false;
        ...
        if (!isSpectator) {
            ...
            ur = onSnapshot(doc(db, 'artifacts', 'mansuke-jinro', 'public', 'data', 'rooms', roomCode, 'players', user.uid, 'secret', 'roleData'), s => {
                if (s.exists()) {
                    ...
                    // 役職指定リクエストの結果があれば通知
                    if (d.roleRequestResult && !roleResultShown) {
                        showNotify(d.roleRequestResult, "success", 6000);
                        roleResultShown = true;
                    }
                }
            }, () => {
                // silent
    ...
    useEffect(() => {
        ...
        const unsubGrave = onSnapshot(query(collection(db, 'artifacts', 'mansuke-jinro', 'public', 'data', 'rooms', roomCode, 'graveChat'), orderBy('createdAt', 'asc')), (snap) => {
            setGraveMessages(snap.docs.map(d => d.data()));
        }, () => {
            // silent
        });

        return () => unsubGrave();
    }, [roomCode, user, isGameEnded, isDead, isSpectator]);
    ...
    useEffect(() => {
        ...
        if (teamChannel) {
            const unsub = onSnapshot(query(collection(db, 'artifacts', 'mansuke-jinro', 'public', 'data', 'rooms', roomCode, 'teamChats'), where('channel', '==', teamChannel)), (snap) => {
                setTeamMessages(snap.docs.map(d => d.data()).sort((a, b) => getMillis(a.createdAt) - getMillis(b.createdAt)));
            }, () => {
                // silent
            });
            return () => unsub();
    ...
    const handleVoteSequenceEnd = () => {
        ...
        if (room.executionResult) {
            ...
            if (isMyExecution) {
                ...
                setOverlay({
                    title: "", subtitle: myDeathContent, duration: 8000, isNight: true, onComplete: () => {
                        setOverlay({ title: "夜になりました", subtitle: "能力者は行動してください", duration: 4000, isNight: true, onComplete: () => setOverlay(null) });
                    }
                });
            } else {
                setOverlay({
                    title: "夜になりました", subtitle: room.executionResult, duration: 5000, isNight: true, onComplete: () => {
                        setOverlay({ title: "夜になりました", subtitle: "能力者は行動してください", duration: 4000, isNight: true, onComplete: () => setOverlay(null) });
                    }
                });
            }
    ...
    const handleSendChat = async (text) => {
        ...
        await fn({ roomCode, collectionName: 'chat', messageData: { text, senderName: myPlayer.name, day: roomDay, phaseLabel: 'day' } });
    };

    const handleSendGraveMessage = async (text) => {
        if (!text.trim()) return;
        const fn = httpsCallable(functions, 'sendChatMessage');
        await fn({ roomCode, collectionName: 'graveChat', messageData: { text, senderName: myPlayer.name } });
    };
    ...
    const handleSendTeamMessage = async (text) => {
        ...
        const fn = httpsCallable(functions, 'sendChatMessage');
        await fn({ roomCode, collectionName: 'teamChats', messageData: { text, senderName: myPlayer.name, channel, day: roomDay, phaseLabel: 'night' } });
    };
    ...
    if (!room || !players || players.length === 0 || (!isSpectator && !myPlayer) || (!isSpectator && !myRole && displayPhase !== 'countdown')) return (
        <LoadingScreen message="ゲームデータを読み込み中..." />
    );

    if (displayPhase === 'countdown') return <CountdownScreen roomCode={roomCode} matchId={room?.matchId} mansukeUser={mansukeUser} room={room} timeLeft={timeLeft} />;
