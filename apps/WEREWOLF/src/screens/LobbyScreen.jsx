...
const TABS = [
    ...
    { id: 'roles', label: '役職指定', icon: Shield, color: 'text-blue-400', border: 'border-blue-500/50', bg: 'bg-blue-900/20' },
                    ...
                    {hasDevPlayer && (
                        <div className="bg-indigo-900/30 border border-indigo-500/30 rounded-2xl p-4 flex flex-col gap-2 shrink-0 animate-fade-in shadow-lg">
                            <div className="flex items-center gap-2">
                                ...
                                <h3 className="font-bold text-indigo-100 text-sm md:text-base">MANSUKEの職員がこの部屋に参加しています！</h3>
                            </div>
                            <div className="pl-11">
                                    ...
                                    <div className="flex items-center gap-3 overflow-hidden">
                                        ...
                                        <div className="flex flex-col min-w-0">
                                            <div className="flex items-center gap-2">
                                                ...
                                                {p.isDev && <span className="text-[10px] bg-indigo-900/50 text-indigo-300 px-1.5 py-0.5 rounded border border-indigo-500/30 flex items-center gap-0.5 shrink-0"><BadgeCheck size={10} /> MANSUKE</span>}
                                            </div>
                    ...
                    <div className="flex-1 overflow-y-auto p-4 md:p-6 custom-scrollbar relative">
                        ...
                        {activeTab === 'roles' && (
                            <div className="space-y-8 animate-fade-in pb-10">
                                {[
                                    { id: 'citizen', name: '村人陣営', color: 'text-blue-400', bg: 'bg-blue-500/10' },
                                    { id: 'werewolf', name: '人狼陣営', color: 'text-red-400', bg: 'bg-red-500/10' },
                                    { id: 'third', name: '第三陣営', color: 'text-orange-400', bg: 'bg-orange-500/10' }
                                ].map(group => (
                                    <div key={group.id} className="space-y-4">
                                        <div className={`flex items-center gap-3 p-3 rounded-2xl ${group.bg} border border-${group.id === 'citizen' ? 'blue' : group.id === 'werewolf' ? 'red' : 'orange'}-500/20`}>
                                            <div className={`w-1.5 h-6 rounded-full ${group.id === 'citizen' ? 'bg-blue-500' : group.id === 'werewolf' ? 'bg-red-500' : 'bg-orange-500'}`}></div>
                                            <h3 className={`font-black tracking-widest ${group.color}`}>{group.name}</h3>
                                            <span className="text-xs text-gray-500 font-bold ml-auto bg-black/20 px-2 py-0.5 rounded-full">
                                            ...
                                            {ROLE_GROUPS[group.id].map(key => {
                                                ...
                                                return (
                                                    <div key={key} className={`relative flex flex-col h-full p-3 rounded-2xl border transition-all ${count > 0 ? "bg-gray-800/80 border-gray-600 shadow-lg" : "bg-gray-900/40 border-gray-800 opacity-60 hover:opacity-100"}`}>
                                                        ...
                                                        <div className={`mb-2 p-2 rounded-xl w-fit shrink-0 ${count > 0 ? (group.id === 'citizen' ? 'bg-blue-500/20 text-blue-400' : group.id === 'werewolf' ? 'bg-red-500/20 text-red-400' : 'bg-orange-500/20 text-orange-400') : "bg-gray-800 text-gray-600"}`}>
