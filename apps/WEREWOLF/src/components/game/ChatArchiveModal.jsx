...
export const ChatArchiveModal = ({ messages, onClose, logs, user }) => {
    // 表示モード判定
    ...
    return (
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-[200] flex items-center justify-center p-4" onClick={onClose}>
            ...
            {/* モーダル本体 */}
            <div className="bg-gray-800/80 border border-gray-700 rounded-3xl w-full max-w-2xl h-[85vh] md:h-[80vh] flex flex-col shadow-2xl overflow-hidden" onClick={e => e.stopPropagation()}>
                {/* ヘッダーエリア */}
                <div className="p-3 md:p-4 border-b border-gray-700 flex flex-col gap-2 shrink-0 bg-gray-950">
                    <div className="flex justify-between items-center">
                        <div className="flex items-center gap-2">
                            <h2 className="text-lg md:text-xl font-black text-gray-200 flex items-center gap-2">
                                <History className="text-red-400" /> チャット履歴
                            </h2>
                        </div>
                        <button onClick={onClose} className="p-1 bg-gray-800 text-gray-300 rounded-full hover:bg-gray-700 transition">
                            <X size={20} />
                        </button>
                    </div>
                    ...
                    {isLogMode && (
                        <div className="flex gap-2 overflow-x-auto custom-scrollbar pb-1">
                            <button
                                onClick={() => setTab('all')}
                                className={`px-3 py-1 rounded-lg text-xs font-bold whitespace-nowrap transition ${tab === 'all' ? 'bg-red-600 text-white' : 'bg-gray-800 text-gray-300 hover:bg-gray-700 border border-gray-700'}`}
                            >
                                すべて
                            </button>
                            {tabs.map(t => (
                                <button
                                    key={t}
                                    onClick={() => setTab(t)}
                                    className={`px-3 py-1 rounded-lg text-xs font-bold whitespace-nowrap transition ${tab === t ? 'bg-red-600 text-white' : 'bg-gray-800 text-gray-300 hover:bg-gray-700 border border-gray-700'}`}
                                >
                                    {t}
                                </button>
                            ))}
                            ...
                            {availableChannels.map(c => (
                                <button 
                                    ...
                                    className={`px-3 py-1 rounded-lg text-xs font-bold whitespace-nowrap transition ${
                                        chatTab===c 
                                            ? (c === 'wolf' ? 'bg-red-600 text-white' : c === 'grave' ? 'bg-purple-600 text-white' : 'bg-red-600 text-white')
                ...
                {/* コンテンツエリア（スクロール可能） */}
                <div className="flex-1 overflow-y-auto p-3 md:p-4 space-y-3 custom-scrollbar bg-gray-950">
                    
                    {/* ログ表示モード */}
                    {isLogMode && safeLogs.filter(l => tab === "all" || l.phase === tab).map((l, i) => {
                        const isSystem = l.phase === "System"; // Assuming system messages have phase "System"
                        
                        if (isSystem) {
                            return (
                                <div key={i} className="flex justify-center my-2">
                                    <div className="bg-gray-800 text-gray-300 px-3 py-2 rounded-xl text-xs md:text-sm border border-gray-700 leading-relaxed shadow-sm">
                        ...
                        return (
                            <div key={i} className="flex flex-col items-start animate-fade-in">
                                {/* フェーズラベル */}
                                <span className="text-[10px] text-gray-300 mb-1 px-1">{l.phase}</span>
                                {/* ログ本文 */}
                    ...
                    {isChatMode && sortedMessages.map((msg, i) => {
                        ...
                        let bubbleStyle = isMe 
                            ? "bg-red-600 text-white rounded-tr-none border-red-600 shadow-sm" 
                            : "bg-gray-800/80 text-gray-100 rounded-tl-none border-gray-600 shadow-sm";
                        
                        // 人狼・霊界チャットの視認性を上げる（自分以外の発言の時）
                        if (!isMe && msg.channel === 'wolf') {
                            bubbleStyle = "bg-red-900/20 text-red-300 border-red-500/40 rounded-tl-none shadow-sm";
                        } else if (!isMe && msg.channel === 'grave') {
                            bubbleStyle = "bg-gray-800 text-gray-200 border-gray-600 rounded-tl-none shadow-sm";
                        }
                    ...
                    {isChatMode && sortedMessages.length === 0 && (
                        <div className="text-center text-gray-300 text-xs md:text-sm py-10">
