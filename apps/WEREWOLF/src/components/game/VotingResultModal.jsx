            ...
            <div className="w-full max-w-3xl flex flex-col h-full max-h-[90vh]">
                ...
                <div className="flex items-center justify-between mb-6 md:mb-8 shrink-0">
                    <div className="flex items-center gap-3 md:gap-4">
                        ...
                        <div>
                            ...
                            <p className="text-xs md:text-base text-gray-300">Voting Results</p>
                        </div>
                    </div>
                    {/* タイマー表示サークル */}
                    <div className="w-14 h-14 md:w-20 md:h-20 rounded-full border-4 border-red-500/30 flex items-center justify-center bg-gray-800/80 shadow-sm shrink-0">
                ...
                <div className="flex-1 overflow-y-auto space-y-3 md:space-y-4 custom-scrollbar pr-2 mb-6 md:mb-8 min-h-0">
                    {voteSummary && voteSummary.map((item, index) => {
                        ...
                        return (
                            <div key={index} className="bg-gray-950 border border-gray-700 rounded-2xl p-3 md:p-4 flex flex-col gap-2 md:gap-3 relative overflow-hidden group shadow-sm transition-all hover:shadow-md">
                                {/* 得票率バー背景アニメーション */}
                                ...
                                <div className="relative z-10 flex items-center justify-between">
                                    <div className="flex items-center gap-3 md:gap-4 min-w-0">
                                        {/* 順位バッジ */}
                                        <div className="w-6 h-6 md:w-8 md:h-8 rounded-full bg-gray-800 border border-gray-600 flex items-center justify-center font-bold text-gray-300 text-xs md:text-sm shrink-0">
                                            {index + 1}
                                        </div>
                                        {/* 被投票者名 (1位は赤文字強調) */}
                                    ...
                                    {/* 票数表示 */}
                                    <span className="text-xl md:text-3xl font-black text-white shrink-0">{item.count}<span className="text-xs md:text-sm font-normal text-gray-300 ml-1">票</span></span>
                                </div>
                                    ...
                                    <div className="relative z-10 flex flex-wrap gap-1 md:gap-2 pl-9 md:pl-12">
                                        {item.voters.map(vid => (
                                            <div key={vid} className="flex items-center gap-1 bg-black/40 px-1.5 py-0.5 md:px-2 md:py-1 rounded-lg border border-white/5">
                                                <User size={10} md:size={12} className="text-gray-300"/>
