...
export const CountdownScreen = ({ roomCode, matchId, mansukeUser, room, timeLeft }) => {
    ...
    return (
        <div className="fixed inset-0 z-[100] flex flex-col md:flex-row bg-gray-950 text-white font-sans overflow-hidden">
            {/* 左側: カウンドダウン & 部屋情報 (30%) */}
            <div className="w-full md:w-[30%] flex flex-col items-center justify-center p-6 md:p-10 border-b md:border-b-0 md:border-r border-gray-800 bg-gray-900/50 backdrop-blur-md relative overflow-hidden">
                {/* 背景装飾 */}
                <div className="absolute top-0 left-0 w-full h-full pointer-events-none opacity-20">
                    <div className="absolute -top-24 -left-24 w-64 h-64 bg-blue-600 rounded-full blur-[80px]"></div>
                    <div className="absolute -bottom-24 -right-24 w-64 h-64 bg-indigo-600 rounded-full blur-[80px]"></div>
                </div>

                <div className="relative z-10 flex flex-col items-center text-center">
                    <div className="flex items-center gap-2 mb-4 bg-blue-500/10 px-4 py-1.5 rounded-full border border-blue-500/30">
                        <Clock className="text-blue-400 animate-pulse" size={18} />
                        <span className="text-xs font-black text-blue-400 tracking-widest uppercase">まもなく開始します...</span>
                    </div>

                    <div className={`text-7xl md:text-9xl font-black font-mono tracking-tighter mb-8 transition-colors duration-300 ${count <= 5 ? 'text-red-500 drop-shadow-[0_0_25px_rgba(239,68,68,0.6)]' : 'text-white drop-shadow-[0_0_20px_rgba(255,255,255,0.3)]'}`}>
                        {count}<span className="text-2xl md:text-4xl ml-2 text-gray-500">s</span>
                    </div>

                    <div className="space-y-3 w-full max-w-[240px]">
                        <div className="flex flex-col gap-1 bg-black/40 p-4 rounded-2xl border border-gray-800 backdrop-blur-sm shadow-xl">
                            <span className="text-[10px] text-gray-500 font-black tracking-widest uppercase">Room Code</span>
                            <span className="text-3xl font-black text-white font-mono tracking-widest">{roomCode}</span>
                        </div>
                        <div className="flex items-center justify-center gap-2 py-2 px-4 bg-gray-800/50 rounded-xl border border-gray-700/50">
                            <Hash size={12} className="text-gray-500" />
                            <span className="text-[10px] font-bold text-gray-400 tracking-tighter">MATCH: {matchId || "---"}</span>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div className="flex-1 flex flex-col p-6 md:p-12 overflow-y-auto custom-scrollbar relative">
                <div className="max-w-4xl w-full mx-auto flex flex-col h-full">
                    ...
                    <div className="mb-8 md:mb-10">
                        ...
                        <div className="space-y-1 text-gray-400 font-bold">
                            ...
                            <p className="text-sm md:text-lg text-white mt-4 bg-gray-900/80 px-4 py-2 rounded-xl inline-block border border-gray-800">
                            ...
                            <div className="flex flex-col h-full">
                                <div className="mb-6 flex items-center gap-3">
                                    ...
                                    <div className="flex flex-col">
                                        <p className="text-lg md:text-xl font-bold text-gray-200">ご希望の役職を選択して、OKを押してください。</p>
                                        <p className="text-sm md:text-base text-gray-500 font-bold mt-1 ml-1">役職を指定しない場合は、OKを押さずにこのままお待ちください。</p>
                                    </div>
                                </div>
                                ...
                                <div className="space-y-8">
                                    <div className="relative">
                                        {isLocked && (
                                            <div className="absolute -inset-4 z-20 bg-gray-950/80 backdrop-blur-md rounded-3xl flex flex-col items-center justify-center border-2 border-emerald-500/30 animate-fade-in shadow-2xl">
                                                ...
                                                <p className="text-gray-400 font-bold">役職の割り振りを待っています...</p>
                                            </div>
                                        )}
                                                ...
                                                return (
                                                    <button
                                                        ...
                                                        className={`relative flex flex-col items-center justify-center gap-2 p-5 rounded-2xl border-2 transition-all duration-200 group ${isSelected
                                                            ...
                                                            : 'bg-gray-900/50 border-gray-800 hover:bg-gray-800 hover:border-indigo-500/50'
                                                            }`}
                                                    >
                                                        <div className={`p-3 rounded-xl transition-colors shrink-0 ${isSelected ? 'bg-indigo-500 text-white' : 'bg-gray-800 text-gray-500 group-hover:text-indigo-400'}`}>
                                                            <Icon size={28} />
                                                        </div>
                                                        <span className={`block font-black text-base md:text-lg text-center ${isSelected ? 'text-white' : 'text-gray-500 group-hover:text-gray-200'}`}>
                                    ...
                                    <div className="flex justify-center mt-8">
                                        <button
                                            ...
                                            className="w-full max-w-md group relative py-6 rounded-3xl font-black text-3xl transition-all active:scale-95 bg-white text-gray-950 hover:bg-emerald-400 hover:text-emerald-950 shadow-[0_0_40px_rgba(255,255,255,0.15)] disabled:opacity-30 disabled:grayscale disabled:scale-95 disabled:cursor-not-allowed flex items-center justify-center gap-4 overflow-hidden"
                                        >
                                            {isSubmitting ? (
                                                <div className="w-8 h-8 border-4 border-gray-300 border-t-gray-950 rounded-full animate-spin" />
                                            ) : (
                                                <>
                                                    <span className="relative z-10">OK</span>
                                                    <Check className="relative z-10" size={28} />
                                                </>
                                            )}
                                            {/* ホバー演出用 */}
